...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, channel = 3):
...
def resnet18(imagenet_pretrained=True, channel=1, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        imagenet_pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [2, 2, 2, 2], channel=channel, **kwargs)
    if imagenet_pretrained:
        model.load_state_dict(torch.load(os.path.join(models_dir, model_name['resnet18'])), strict=False)
    return model

def resnet34(imagenet_pretrained=True, channel=1, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        imagenet_pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [3, 4, 6, 3], channel=channel, **kwargs)
    if imagenet_pretrained:
